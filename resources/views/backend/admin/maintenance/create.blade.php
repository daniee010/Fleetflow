...
@section('content')
    <h1 class="text-2xl font-bold mb-6">Add Maintenance</h1>

    <form action="{{ route('admin.maintenance.store') }}" method="POST" class="space-y-4 max-w-xl">
        @csrf
        <div>
        ...
        <div>
            ...
            <input type="date" name="service_date" class="w-full border rounded p-2" value="{{ old('service_date') }}">
        ...
        <select name="service_type" class="w-full border rounded p-2">
            ...
            @foreach($types as $type)
                <option value="{{ $type }}" @selected(old('service_type') === $type)>
                    {{ Str::headline($type) }}
                </option>
            @endforeach
        </select>
        ...
        <div>
            ...
            <input type="number" step="0.01" name="cost" class="w-full border rounded p-2" value="{{ old('cost') }}">

    ...
    <form method="POST" action="{{ route('admin.expenses.store') }}"
        ...
        <label class="block">
            ...
            <input type="number" step="0.01" min="0" name="amount"
                   value="{{ old('amount','0') }}" class="mt-1 w-full rounded border p-2 bg-transparent">
        </label>
        ...
        <label class="block md:col-span-2">
            ...
            <textarea name="notes" rows="3" class="mt-1 w-full rounded border p-2 bg-transparent"
                      placeholder="Optional notes...">{{ old('notes') }}</textarea>
        </label>

        <div class="md:col-span-2 flex gap-3">
            <button class="px-4 py-2 rounded bg-[#f53003] text-white hover:bg-black">Save</button>
